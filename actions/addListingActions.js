...
const addFoundPerson = (values) => async (dispatch) => {
  ...
  client.post('/foundPerson', form).then(res => {
    ...
    if (res.ok) {
      ...
      axios.post('https://recognize-missing-person.herokuapp.com/find/', {
...
const addFoundDocument = (values) => async (dispatch) => {
  ...
  if (response.ok) return dispatch({ type: "ADD_LISTING_SUCCESS", payload: response.data?.message })
...
const addLostDocuments = (values) => async (dispatch) => {
  ...
  if (!response.ok) return dispatch({ type: 'ADD_LISTING_FAILED', payload: response.data?.message })
  dispatch({ type: "ADD_LISTING_SUCCESS" })
}
